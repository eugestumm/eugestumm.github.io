...
def generate_conferences_section(data):
    """Generate Conference Presentations section with proper handling of organizer roles"""
    ...
    if 'Conferences' in data:
        ...
        for role in roles:
            ...
            for _, row in role_df.iterrows():
                # For organizers, use event_name if title is empty
                title = row.get('title', '')
                event_name = row.get('event_name', '')
                
                # If title is empty but we have event_name (common for organizers), use event_name
                if not title and event_name:
                    display_title = f"{event_name}"
                elif title:
                    display_title = title
                else:
                ...
                # Conference details (only if different from title for organizers)
                if event_name and title and event_name != title:
                    if event_theme:
                        content += f"{event_name}: *{event_theme}*"
                    else:
                ...
                if location_parts:
                    content += f"{', '.join(location_parts)}"
                
                if month and year:
                    if location_parts:
                        content += f", {month} {format_date(year)}"
                    else:
                        content += f"{month} {format_date(year)}"
                content += "\n"
